...
JWT, RapidAPI local OpenAPI MCP, and protocol headers: [examples/10-mcp.md](examples/10-mcp.md). Tool names: [mcp-tools.md](mcp-tools.md).
...
RapidAPI: `POST https://tradingview-data1.p.rapidapi.com/api/mcp/generate` with `x-rapidapi-host` / `x-rapidapi-key`. Local `npx -y @ivotoby/openapi-mcp-server` is REST-shaped tools, **not** hosted `tradingview_*`.
