...
- **Required**: Access to company financial data
  - For public companies: MCP annual history plus latest 10-K / recent 10-Qs for audit and gap-fill
...
- [ ] Have access to TradingView MCP or 10-K filings (public company)?
...
**IF VERIFICATION FAILS**: Stop and obtain access to hosted TradingView MCP, financial statements, or 10-K / equivalent source before proceeding.
...
1. **Seed the historicals from TradingView MCP**
   - Pull `tradingview_get_market_data(symbol, category='financials_annual')`
   - Pull `tradingview_get_market_data(symbol, category='history_annual')`
   - Pull `tradingview_get_market_data(symbol, category='cash_flow')`
