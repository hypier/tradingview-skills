...
- **GET /api/metadata/world-economy/indicators** - Get valid world economy indicator slugs
...
Retrieve fundamentals and company-specific datasets from one endpoint family.
- **GET /api/market-data/{symbol}** - Get the complete categorized market data payload
- **GET /api/market-data/{symbol}/company** - Company profile, industry, website, exchange, employee count
- **GET /api/market-data/{symbol}/ipo** - IPO details
...
**Typical use cases:**
- Fundamentals-first single-stock analysis
- Valuation and quality comparisons
- Dividend strategy research
- Analyst expectation tracking

### 🎯 Screener
Use the screener endpoint family for custom scans beyond fixed leaderboard tabs.
...
1. Confirm market and asset type
2. Fetch presets
3. Fetch filter options
4. Build `preset_fields`, `filters`, and `sort`
5. Run scan and verify top results with quote or TA
...
### 💡 Community Ideas
Access crowd-generated trade ideas and community narrative from TradingView.
- **GET /api/ideas/hot** - Hot ideas feed
- **GET /api/ideas/editors-picks** - Editor-picked ideas
- **GET /api/ideas/{symbol}/minds** - Symbol-specific community direction
- **GET /api/ideas/list/{symbol}** - Paginated list of ideas for a symbol
- **GET /api/ideas/{imageUrl}** - Idea detail page by `image_url`

**Common Parameters:**
- `page` - page number for list endpoints
- `per_page` - results per page for symbol list
- `lang` - language code

**Use cases:**
- Sentiment pulse on a symbol
- Collecting discretionary chart narratives
- Comparing community bias vs quantitative signals

### 🌍 World Economy
Track macro indicators across countries and regions.
- **GET /api/world-economy/indicators/{indicator}** - Get country rankings for a world-economy indicator

**Parameters:**
- `indicator` - indicator slug from `/api/metadata/world-economy/indicators`
- `region` - one of `g20`, `world`, `north-america`, `europe`, `middle-east-africa`, `latin-america`, `asia-pacific`

**Example:**
- `/api/world-economy/indicators/full-year-gdp-growth?region=g20`
...
### 📡 SSE Real-time Data
The same generated token can also be used with SSE streaming for live monitoring dashboards and alerting flows.

Typical pattern:
1. `POST /api/token/generate`
2. Connect to SSE `/sse/stream`
3. Subscribe to symbols or channels
4. Consume incremental updates
...
- **Fundamental Coverage**: Company profiles, financial statements, dividends, analyst recommendations
- **Custom Screening**: Preset + filter-option based screener workflow
- **Community Insight**: TradingView ideas and symbol minds
- **Macro Coverage**: World economy indicators and economic calendar
...
Use these exact IDs. `income_statement`, `balance_sheet`, `cash_flow`, and `technical` are invalid for leaderboard. Screener preset ids are separate and stay snake_case (`income_statement`, `technicals`).
...
## World Economy Indicator Metadata

Use `GET /api/metadata/world-economy/indicators` to retrieve valid indicator slugs before querying rankings.

Typical indicator categories include:
- GDP growth
- Inflation
- Interest rates
- Unemployment
...
# Get world economy indicator slugs
...
```bash
# Company profile
GET /api/market-data/NASDAQ:AAPL/company

# Quarterly financials
GET /api/market-data/NASDAQ:AAPL/financials-quarterly

# Analyst recommendations
GET /api/market-data/NASDAQ:AAPL/analyst-recommendations
```
...
POST /api/screener/scan
{
  ...
  "filters": {
    "price_earnings_ttm": { "operation": "less_or_equal", "value": 20 },
    "RSI": { "operation": "less_or_equal", "value": 40 }
  }
}
```
...
# Symbol idea list
GET /api/ideas/list/BINANCE:BTCUSDT?page=1&per_page=20&lang=en

# Symbol minds
GET /api/ideas/BINANCE:BTCUSDT/minds?lang=en
```

### Get World Economy Rankings
```bash
# GDP growth ranking for G20
